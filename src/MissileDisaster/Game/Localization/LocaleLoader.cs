...
{
    /// <summary>
    /// Loads Locales/&lt;lang&gt;.txt from the mod folder and overwrites MissileStrings' fields by
    /// reflection. See MissileStrings for the scheme.
    ///
    ...
    internal static class LocaleLoader
    {
        ...
        {
            FieldInfo[] fields = typeof(MissileStrings).GetFields(BindingFlags.Public | BindingFlags.Static);
        ...
        private static void EnsureTemplate(string dir)
        {
            try
            {
                ...
                {
                    w.WriteLine("# Missile Disaster UI strings (English template).");
                    ...
                    w.WriteLine("# Contributions welcome: https://github.com/omoneko/MissileDisaster");
                    w.WriteLine();
                    foreach (FieldInfo f in StringFields())
